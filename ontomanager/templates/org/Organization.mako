...
<%
    organization = CACHE[U["org"]["show"]["qname"]]
    imgUrl = request.static_url("ontomanager:static/models/org/Company/%s.png" %(organization['short_name']))

    manufactured = [ CACHE[m] for m in organization['manufactured'] ]
%>


<h1>Company ${misc.render_view_link(organization, "title", contents=organization['short_name'])}</h1>
